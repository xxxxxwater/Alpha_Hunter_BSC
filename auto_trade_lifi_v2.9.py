...
# 动态导入v2.9模块
spec = importlib.util.spec_from_file_location("alpha_hunter_v29", "alpha_hunter_lifi_v2.9.py")
alpha_hunter_v29 = importlib.util.module_from_spec(spec)
spec.loader.exec_module(alpha_hunter_v29)
AlphaHunter = alpha_hunter_v29.AlphaHunter
...
def main():
    print("\n" + "=" * 60)
    print("          Alpha Hunter - LI.FI版 v2.9")
    print("          强化限流保护，智能自动止盈")
    print("=" * 60)
    print()
    
    # 读取私钥
    private_key = os.getenv('PRIVATE_KEY') or os.getenv('BSC_PRIVATE_KEY')
    if not private_key:
        print("[错误] 未设置PRIVATE_KEY或BSC_PRIVATE_KEY")
        print()
        print("请在 .env 文件中设置:")
    ...
    try:
        ...
        if check_interval < 60:
            print(f"[警告] 检查间隔 {check_interval}秒 可能导致API限流")
            print(f"[建议] 设置CHECK_INTERVAL=60或更高")
            print()
